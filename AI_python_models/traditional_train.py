...
from traditional_pipelines import get_pre_tfidf, get_pre_tfidf_svd, get_pre_bow, get_pre_tfidf_chi

X_train, X_val, X_test, y_train, y_val, y_test = data_split()

# Define los modelos
model_pipelines = {
    'SVM'    : ImbPipeline([('pre', get_pre_tfidf_chi()),     ('smote', SMOTE(random_state=200900)),   ('clf', LinearSVC(class_weight='balanced', max_iter=10000, random_state=200900))]),
    'LogReg' : ImbPipeline([('pre', get_pre_tfidf_chi()),     ('smote', SMOTE(random_state=200900)),   ('clf', LogisticRegression(class_weight='balanced', solver='saga', max_iter=5000, random_state=200900))]),
    ...
    'HGB'     : ImbPipeline([('pre', get_pre_tfidf_chi()), ('smote', SMOTE(random_state=200900)),   ('clf', HistGradientBoostingClassifier(early_stopping=True, validation_fraction=0.1, tol=1e-7, random_state=200900))]),
